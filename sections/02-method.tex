...
task and then view the agent's observation and actions step by step, in text form.\footnote{While viewing screenshots is standard for web navigation tasks, we keep
...
\textbf{Feedback Grounding}
The feedback of human annotators can contain multiple aspects; e.g. \textsl{``AI
...
grounding procedure is similar to the coding procedure in thematic analysis.

We feed the trajectory
...
The final loop component is the meta-evaluation, i.e. evaluating the
...
As illustrated in Fig. \ref{fig:autolibra_optimization}, we optimize the metric
...
this optimization process is simple, experiments with various other  \begin{wrapfigure}
	[22]{r}{0.5\textwidth}
	\vspace{-10pt}
...
crucial in the metrics, dropping which resulting in up to $30\%$ coverage decrease on CoGym. % \citep{shao2024collaborative}).
